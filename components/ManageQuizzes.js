...
const ManageQuizzes = ({ onClose, onQuizActivated, quizzes, setQuizzes, orderModes, setOrderModes }) => {
  ...
  const [dbStatus, setDbStatus] = useState('Fetching quizzes...')
    ...
    async function fetchDbQuizzes() {
      try {
        ...
        if (!data || data.length === 0) {
          setDbStatus('No quizzes found in the database')
          setDbQuizzes([])
          return
        }
        ...
        setDbStatus('Quizzes fetched successfully')
        ...
        setDbStatus(`Failed to fetch quizzes: ${error.message}`)
      }
    }
    ...
    <div className="manage-quizzes-overlay">
      <div className="manage-quizzes-container">
        ...
        <div className="quizzes-list">
          <p className="mb-2">Database Status: <span className={dbStatus.includes('success') ? 'text-green-600' : 'text-red-600'}>{dbStatus}</span></p>
          {allQuizzes.length === 0 ? (
